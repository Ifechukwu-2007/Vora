...
async function initializeProviderHub() {
  ...
  user = authUser;
  providerName.textContent = authUser.user_metadata?.full_name || authUser.email || "Provider";
  ...
  activeOffersList.innerHTML = active.map(o => `
    <div class="border p-3 rounded mb-2 bg-white">
      ...
      <div class="flex gap-2 mt-2 flex-wrap">
        <button onclick="window.openOffer('${o.id}')"
          class="px-3 py-1 bg-gray-100 rounded text-sm">
          View
        </button>

        ${o.status === STATUS.PENDING ? `
          <button onclick="window.acceptOffer('${o.id}')"
            class="px-3 py-1 bg-green-500 text-white rounded text-sm">
            Accept
          </button>

          <button onclick="window.rejectOffer('${o.id}')"
        ...
        ${o.status === STATUS.ACCEPTED ? `
          <button onclick="window.markAwaitingPayment('${o.id}')"
        ...
        ${o.status === STATUS.AWAITING_PAYMENT ? `
          <button onclick="window.markCompleted('${o.id}')"
            class="px-3 py-1 bg-blue-600 text-white rounded text-sm">
            Mark Completed
          </button>
        ` : ""}

        <button onclick="window.cancelOffer('${o.id}')"
  ...
  modalContent.innerHTML = `
    ...
    <div class="mt-4 space-y-2">
      ...
      ${offer.status === STATUS.ACCEPTED ? `
        <button onclick="window.markAwaitingPayment('${offer.id}')"
      ...
      ${offer.status === STATUS.AWAITING_PAYMENT ? `
        <button onclick="window.markCompleted('${offer.id}')"
          class="w-full bg-green-600 text-white py-2 rounded">
          Mark as Completed
        </button>
      ` : ""}

      <button onclick="window.cancelOffer('${offer.id}')"
